...
const ImageUploadModal = ({ closeModal, imageData }) => {
    const { register, handleSubmit, formState: { errors } } = useForm()
    const [openFastLinkModal, setOpenFastLinkModal] = useState(false)
    const key = '0ea873c4c7476f789bc1d4c4e7b5545b';
            ...
            <div className='p-3'>
                ...
                <form onChange={handleSubmit(ImageUpload)}>
                    <div className='grid grid-cols-2 gap-6 mt-3'>
                        <div onClick={() => closeModal(false)} className="relative w-24 h-24 flex justify-center items-center bg-gray-200 hover:bg-blue-600 rounded-md">
                            <label className="cursor-pointer flex justify-center items-center">
                                <div className=" relative flex cursor-pointer items-center justify-center">
                                    ...
                                    <input {...register('image', { required: 'image is Required' })}
                                        type="file" name='image' className="w-6 absolute opacity-0 cursor-pointer" />
                                </div>
                            </label>
                        </div>
