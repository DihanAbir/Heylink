...
import { setPromptReducer } from "../../Slices/apiSlice";
...
const Templates = () => {
  ...
  const clickHandler = () => {
    alert("will be changed");
    dispatch(setPromptReducer({ userPrompt: "changed data?" }));
  };
  return (
    <section className="bg-[#393AA7]">
      ...
      <div className="text-center text-white px-8">
        ...
        <button
          className=" bg-[#F67261]  items-center gap-6 border-2 border-white text-white py-3 px-4 rounded-[50px]"
          onClick={clickHandler}
        >
          Click to change redux
        </button>
      </div>
      ...
      <div className="hidden lg:block lg:grid lg:grid-cols-6 gap-4 mx-auto py-8 px-12 cursor-pointer">
        {categories.map((category) => {
          return (
            <div
              onClick={() => setViewCategory(category.id)}
              className={`flex justify-center items-center border-2 border-white text-white hover:text-sky-600 hover:bg-white lg:py-2 lg:px-4 rounded-[50px] font-semibold lg:text-[16px] ${
                viewCategory === category.id && "bg-white text-sky-600"
              }`}
            >
